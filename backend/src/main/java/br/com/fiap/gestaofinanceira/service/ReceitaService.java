...
public class ReceitaService {
    ...
    public Receita atualizar(Long id, Receita receitaAtualizada) {
        Optional<Receita> optionalReceitaExistente = receitaRepository.findById(id);

        if (optionalReceitaExistente.isPresent()) {
            Receita receitaExistente = optionalReceitaExistente.get();

            receitaExistente.setDescricao(receitaAtualizada.getDescricao());
            receitaExistente.setValor(receitaAtualizada.getValor());
            receitaExistente.setDataRecebimento(receitaAtualizada.getDataRecebimento());

            return receitaRepository.save(receitaExistente);
        } else {
            throw new RuntimeException("Receita não localizada");
        }
    }
}